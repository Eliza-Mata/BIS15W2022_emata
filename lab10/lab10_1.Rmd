...
```{r}
elephants %>%
  ggplot(aes(x=sex)) + geom_bar()+coord_flip()
```
...
homerange %>%
  ...
  labs(title= "thermoregulation", x="thermoregulation", y=NULL )
```
...
```{r}
homerange %>% 
  ggplot(aes(x=log10.mass, y=log10.hra, size=log10.hra))+
  geom_point(na.rm=T)+labs(title="log_mass and homerange_log_mass", y="homerange", x="log_mass")
```
